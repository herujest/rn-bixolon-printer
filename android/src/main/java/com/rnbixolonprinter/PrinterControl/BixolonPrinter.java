...
public class BixolonPrinter implements ErrorListener, OutputCompleteListener, StatusUpdateListener, DirectIOListener, DataListener {
    ...
    public boolean setPDFLicenseKey(String key) {
        try {
            // bixolon_pdf.aar
            Class<?> cls = Class.forName("com.bixolon.pdflib.PdfCore");
            posPrinter.setPDFLicenseKey(key);
            return true;
        } catch (ClassNotFoundException | JposException e) {
            e.printStackTrace();
        }
        return false;
    }
